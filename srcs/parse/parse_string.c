...
int						ft_atoi_strict(char *str)
{
	...
	while (*str)
	{
		if (ft_isdigit(*str) == FALSE)
			throw_error_num("Wrong integer number character in .rt file", EIO);
		ans *= 10;
		ans += (*str - '0');
		str++;
	}
...
double					ft_atod_strict(char *str)
{
	...
	while (nums[len] != NULL)
		len++;
	if (len == 0)
		throw_error_num("Wrong double number character in .rt file", EIO);
...
t_vec					*vec_atod(char *str)
{
	...
	while (nums[len] != NULL)
		len++;
	if (len != 3)
		throw_error_num("Wrong double vector string in .rt file", EIO);
	len = -1;
	while ((++len) < 3)
...
t_vec					*vec_atoi(char *str)
{
	...
	while (nums[len] != NULL)
		len++;
	if (len != 3)
		throw_error_num("Wrong integer vector string in .rt file", EIO);
	len = -1;
	while ((++len) < 3)
